...
from .models import MovieReviewCache
...
headers = {
        ...
        "Authorization": "Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiIxNDJjYTg1OTJkYzIwMzMyZWVlNzhkYjFlY2ZkYjkxYiIsInN1YiI6IjY2NTkzNWM3NTNlYmQ1ZDY4Njk0MTE3MiIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.gWkKKQy67Q-z3Y7V-EDNzdnFojMLNHOdonGE3LmRtPY",
    }
...
def set_language(request):
    ...
    if request.method == 'POST':
        ...
        current_language = "ko-" + data.get('language', 'ko-KR')
...
async def gather_user_urls(movie_reviews_url):
    async with aiohttp.ClientSession() as session:
        tasks = [fetch_reviews(session, movie_reviews_url, i) for i in range(1, 15)]
        pages_results = await asyncio.gather(*tasks)
        user_url_list = [link for sublist in pages_results for link in sublist if sublist]
    return user_url_list

async def fetch_reviews(session, url, index):
    try:
        full_url = f"{url}{index}"
        html = await fetch(session, full_url)
        soup = BeautifulSoup(html, "html.parser")
        viewings_list = soup.find_all("li", class_="film-detail")
        user_links = ["https://letterboxd.com" + li_tag.find('a')['href'] for li_tag in viewings_list]
        return user_links
    except Exception as e:
        print(f"Failed to fetch or parse page {index}: {str(e)}")
        return []

async def fetch_favorite_films(session, user_url):
    try:
        html = await fetch(session, user_url)
        soup = BeautifulSoup(html, "html.parser")
        film_slugs = [div['data-film-slug'] for div in soup.select("section#favourites div.really-lazy-load")
                      if 'data-film-slug' in div.attrs]
        return film_slugs
    except Exception as e:
        print(f"Error processing user profile {user_url}: {str(e)}")
        return []

async def gather_user_favorites(user_urls):
    async with aiohttp.ClientSession() as session:
        tasks = [fetch_favorite_films(session, user_url) for user_url in user_urls]
        results = await asyncio.gather(*tasks, return_exceptions=True)
        return results

@xframe_options_exempt
@async_only_middleware
async def movie_reviews(request, id):
    context = await fetch_movie_reviews(request, id)
    print(context.get('combined_details')[0])
    return render(request, 'movies/recommendations.html', context)

async def fetch_movie_reviews(request, id):
    # 데이터베이스에서 캐시된 데이터를 조회
    try:
        cached_review = await sync_to_async(MovieReviewCache.objects.get)(movie_id=id)
        print("Cache hit")
        return cached_review.context
    except MovieReviewCache.DoesNotExist:
        print("Cache miss")
        pass

    context = {}
    url = f"https://api.themoviedb.org/3/movie/{id}?language={current_language}"
    response = requests.get(url, headers=headers)
    data = json.loads(response.text)
    context = {'movies': data}
    
    search = data.get('title')
    search_url = f"https://letterboxd.com/search/films/{search}/"
    response = requests.get(search_url)
    soup = BeautifulSoup(response.text, 'html.parser')
    ul_tag = soup.find('ul', class_='results')
    li_tag = ul_tag.find_all('li', recursive=False)
    for li in li_tag:
        year = li.find('small', class_='metadata').text
        if (year == data.get('release_date')[:4]):
            movie_link = li.select_one('div.really-lazy-load.poster.film-poster.linked-film-poster')['data-film-slug']
            break
    
    movie_url = f"https://letterboxd.com/film/{movie_link}/reviews/by/entry-rating/page/"
    user_url_list = await gather_user_urls(movie_url)
    recommend_movie_list = await gather_user_favorites(user_url_list)
    recommend_movie_list = [item for sublist in recommend_movie_list for item in sublist]
    title_counts = {title: recommend_movie_list.count(title) for title in set(recommend_movie_list)}
    sorted_titles = sorted(title_counts.items(), key=lambda x: x[1], reverse=True)[:20]
    async def fetch_movie_detail(title, count, index_url):
        index_response = requests.get(index_url)
        index_soup = BeautifulSoup(index_response.text, 'html.parser')
        index_id = index_soup.find('body')['data-tmdb-id']
        return (title, count, index_id)
    print("fetch_movie_detail end")
    tasks = [fetch_movie_detail(title, count, f"https://letterboxd.com/film/{title}") for title, count in sorted_titles]
    updated_sorted_titles = await asyncio.gather(*tasks)
    # 여기까진 속도가 빠름
    async def fetch_movie_context(id):
        url = f"https://api.themoviedb.org/3/movie/{id}?language={current_language}"
        response = requests.get(url, headers=headers)
        data = json.loads(response.text)
        movie_id = data.get('id')
        if movie_id:
            youtube_url = get_youtube_url_by_id(movie_id)
            if youtube_url:
                data['youtube_url'] = "https://www.youtube.com/embed/" + youtube_url + "?rel=0&wmode=transparent"
            else:
                data['youtube_url'] = None
        return {'movies': data}
    print("fetch_movie_context end")
    context_detail_tasks = [fetch_movie_context(id) for _, _, id in updated_sorted_titles]
    context_detail = await asyncio.gather(*context_detail_tasks)
    combined_details = [{'title': title, 'count': count, 'id': id, 'details': detail['movies']} for (title, count, id), detail in zip(updated_sorted_titles, context_detail)]
    context = {'combined_details': combined_details, 'movie_url': movie_url, 'movie_title': movie_link}
    # 데이터베이스에 캐시된 데이터를 저장
    await sync_to_async(MovieReviewCache.objects.create)(movie_id=id, context=context)

    return context

def main_page(request):
    countries = get_country()
    return render(request, 'movies/main.html', countries)

def search_form(request):
    return render(request, 'movies/search_form.html')

def get_youtube_url_by_id(id):
    url = f"https://api.themoviedb.org/3/movie/{id}/videos?language={current_language}"
    response = requests.get(url, headers=headers)
    data = json.loads(response.text)
...
def search_movie_by_name(title, year):
    context = {}
    url = f"https://api.themoviedb.org/3/search/movie?query={title}&include_adult=false&language={current_language}&page=1"
    response = requests.get(url, headers=headers)
    data = json.loads(response.text)
    movies = data.get('results', [])
        
    for movie in movies :
        if movie.get('release_date')[:4] == year:
            movie_id = movie.get('id')
            if movie_id:
                youtube_url = get_youtube_url_by_id(movie_id)
                if youtube_url:
                    movie['youtube_url'] = "https://www.youtube.com/embed/" + youtube_url + "?rel=0&wmode=transparent"
            else:
        ...
        for movie in movies :
            ...
            if movie_id:
                ...
                if youtube_url:
                    ...
                    movie['youtube_url'] = None
        context = {'movies': movies}
    return render(request, 'movies/search_form.html', context)
...
def get_movie_info(request, id):
    ...
    response = requests.get(url, headers=headers)
    data = json.loads(response.text)
    data['providers'] = get_watch_providers(id)
    data['credits'] = get_credits(id)
    context = {'movies': data}
    return render(request, 'movies/info.html', context)

def get_movie_recommendations(movie_id):
    print(f"get_movie_recommendations: {movie_id}")
    cache_key = f"movie_{movie_id}_recommendations"
...
def get_credits(movie_id):
    url = f"https://api.themoviedb.org/3/movie/{movie_id}/credits?language={current_language}"
    response = requests.get(url, headers=headers)
    data = json.loads(response.text)
    return data
